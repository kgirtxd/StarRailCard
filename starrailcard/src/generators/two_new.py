...
class Creat:
    ...
    async def creat_bacground(self):
        ...
        if self.img:
            # user_image = await pill.creat_user_image_tree(self.img)
            user_image = Image.new("RGBA",(782,782), (0,0,0,0))
            art = await pill.get_centr_honkai_art((582,782),self.img)
            user_image.paste(art,(200,0))
        ...
        if not self.hide:
            ...
            self.background.alpha_composite(uid,(207,768))
    ...
    async def build(self):
        ...
        self.background.alpha_composite(self.background_name,(206,654)) #(6,654))
    ...
    async def remove_left(self, pixles):
        width, height = self.background.size
        box = (pixles, 0, width, height)
        cropped_image = self.background.crop(box)
